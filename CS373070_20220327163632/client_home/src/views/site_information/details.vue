<template>
	<div class="diy_details page_site_information" id="site_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_site_information'>
							<div_site_information :obj="obj"></div_site_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_site_information from "@/components/diy/div_site_information.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/site_information/set?site_information_id=' + obj["site_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						console.log("添加访问量状态：" ,res.result);
						var body = {
							source_table: "site_information",
							source_field: "site_information_id",
							source_id: this.obj.site_information_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "site_information",
					source_field: "site_information_id",
					source_id: obj["site_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/site_information/set?site_information_id=' + _this.obj["site_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){

		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/site_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_site_information = !show_list_site_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_site_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--场地信息推荐列表组件 -->
		                <list_site_information v-if="show_list_site_information" :list="list_site_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_site_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_site_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_site_information" :key="i" @click="to_details('site_information',o,'site_information_id')">
		                            <td v-for="(oj,n) in fields_site_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_site_information: true,
				list_site_information: [],
				vm_arr_site_information: [
					"site_information_id",
					"site_number",
					"site_type",
					"site_name",
		...
		methods: {
			...
			get_site_information() {
				let url = "~/api/site_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_site_information = json.result.list;
					}
				})
			},
			// 获取比赛项目列表
		...
		computed: {
		    list_table_site_information() {
		        var list = this.list_site_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_site_information.map((o) => {
		                // 第二个中括号是对象的属性

<template>
	<div class="diy_details_box diy_div_volunteer">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "volunteer_id",
					source_id: this.obj["volunteer_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/volunteer/set?volunteer_id=' + _this.obj["volunteer_id"], {
								praise_len
							}, (res) => {
								if(res.result){

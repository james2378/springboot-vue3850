<template>
	<div class="diy_details_box diy_div_referee_allocation">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "referee_allocation",
					source_field: "referee_allocation_id",
					source_id: this.obj["referee_allocation_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/referee_allocation/set?referee_allocation_id=' + _this.obj["referee_allocation_id"], {
								praise_len
							}, (res) => {
								if(res.result){

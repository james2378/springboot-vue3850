<template>
	<div class="diy_details_box diy_div_athletes">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "athletes",
					source_field: "athletes_id",
					source_id: this.obj["athletes_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/athletes/set?athletes_id=' + _this.obj["athletes_id"], {
								praise_len
							}, (res) => {
								if(res.result){

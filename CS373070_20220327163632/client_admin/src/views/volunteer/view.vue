			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['volunteer_id'] && $check_field('set','gender')) || (!form_sub['volunteer_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','age') || $check_field('add','age') || $check_field('set','age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="年龄" prop="age">
					<el-input id="age" v-model="form_sub['age']" placeholder="请输入年龄"
							  v-if="user_group === '管理员' || (form_sub['volunteer_id'] && $check_field('set','age')) || (!form_sub['volunteer_id'] && $check_field('add','age'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["volunteer_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?volunteer_id=" + form_sub["volunteer_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/volunteer/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/volunteer/view','get');
